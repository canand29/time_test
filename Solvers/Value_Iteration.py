...
class AsynchVI(ValueIteration):
    ...
    def train_episode(self):
        ...
        # for t in range(self.env.observation_space.n):
            state = self.pq.pop()
            # print('1')
            ...
            for each_state in self.pred[state]:
                # print('2')
                action_values = self.one_step_lookahead(each_state)
                best_action = np.max(action_values)

                priority = -abs(self.V[each_state] - best_action) # needs to be negative
                self.pq.update(each_state, priority)
