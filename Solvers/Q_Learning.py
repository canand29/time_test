...
class ApproxQLearning(QLearning):
    ...
    def train_episode(self):
        ...
        while not done:
            ...
            next_q_value = self.estimator.predict(next_state)

            # Q(S,A) ← Q(S,A) + alpha [R + gamma * Q(next_state, next_action) - Q(S,A)]
            updated_q_value = q_value + self.options.alpha * (reward + self.options.gamma * np.max(next_q_value) - q_value)
            self.estimator.update(state, action, updated_q_value)
